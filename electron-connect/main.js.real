...
const DLD_OWNER_LOGIN_URL = "https://dubailand.gov.ae/en/MyDLD/#/login/owner";
...
function resolveDesktopLoginUrl() {
  if (launchState.flow === "fbr") {
    const configured = launchState.desktopAuthConfig.loginUrl || "mock-iris://login";
 ...
 */
async function navigateIrisTopMenu(windowInstance, routeSelector, driftContext) {
  ...
  try {
    await trySelectorsInPriority(windowInstance, "topMenuSelector", selectors, driftContext);
  } catch (error) {
 ...
 */
async function navigateIrisLeftCategory(windowInstance, routeSelector, driftContext) {
  if (!routeSelector?.leftCategorySelector) {
    return false;
  }

  const selectors = buildActionSelectorChain(routeSelector, "leftCategorySelector");

  try {
    await trySelectorsInPriority(windowInstance, "leftCategorySelector", selectors, driftContext);
  } catch (error) {
...
async function verifyFormReady(windowInstance, routeSelector) {
  ...
  try {
    await waitForVisibleSelector(windowInstance, routeSelector.formReadySelector, 10000);
    return true;
  } catch {
    return false;
  }
...
async function setIrisFormPeriod(windowInstance, routeSelector, taxYear) {
  ...
  try {
    await fillSelector(windowInstance, routeSelector.periodSelector, String(taxYear));
    return true;
  } catch {
...
async function handleIrisFormName(windowInstance, routeSelector, taxpayerName) {
  ...
  try {
    await fillSelector(windowInstance, routeSelector.nameSelector, String(taxpayerName));
    return true;
  } catch {
    return false;
  }
 ...
 */
async function navigateToIrisForm(windowInstance, routeSelector, formLabel, taxYear, taxpayerName) {
  const steps = [];
  ...
  const formFound = await navigateIrisFormList(windowInstance, routeSelector, formLabel);
  if (formFound) {
  ...
  if (taxYear) {
    const periodSet = await setIrisFormPeriod(windowInstance, routeSelector, taxYear);
    if (periodSet) {
  ...
  if (taxpayerName) {
    const nameSet = await handleIrisFormName(windowInstance, routeSelector, taxpayerName);
    if (nameSet) {
      steps.push("name_set");
    }
  }
...
async function handlePsidGeneration(windowInstance, routeSelector) {
  ...
  const hasGenerateButton = await hasSelector(windowInstance, routeSelector.generatePsidSelector);
...
async function verifyCprAndPayment(windowInstance, routeSelector) {
  ...
  if (routeSelector.cprDisplaySelector) {
    try {
      result.cprDetected = await hasSelector(windowInstance, routeSelector.cprDisplaySelector);
      if (result.cprDetected) {
        result.cprReference = await windowInstance.webContents.executeJavaScript(`
          (() => {
  ...
  if (routeSelector.balancePayableSelector) {
    try {
      result.balancePayable = await windowInstance.webContents.executeJavaScript(`
        (() => {
          const element = document.querySelector(${JSON.stringify(routeSelector.balancePayableSelector)});
          return element ? (element.textContent || element.value || "").trim() : null;
        })();
  ...
  if (routeSelector.refundBannerSelector) {
    try {
      result.refundDetected = await hasSelector(windowInstance, routeSelector.refundBannerSelector);
    } catch {
...
async function verifyCompletionEvidence(windowInstance, routeSelector) {
  ...
  if (routeSelector?.completionConfirmSelector) {
    try {
      evidence.completedTasksDetected = await hasSelector(windowInstance, routeSelector.completionConfirmSelector);
    } catch {
      // Ignore
    }
  }
  ...
  const evidenceSelectors = {
    acknowledgementDetected: '#iris-acknowledgement-proof, [id*="acknowledgement"], [class*="acknowledgement"]',
    returnCopyDetected: '#iris-return-copy-proof, [id*="returnCopy"], [class*="return-copy"]',
...
function isLikelyLoggedInDldUrl(rawValue) {
  ...
  try {
    ...
    const hostMatches = parsed.hostname === DLD_PORTAL_HOST || parsed.hostname.endsWith(`.${DLD_PORTAL_HOST}`);
...
function applyLaunchUrl(rawValue) {
  ...
  const parsed = new URL(rawValue);
  launchState = {
    flow: parsed.searchParams.get("flow") === "fbr" ? "fbr" : "dld",
    token: parsed.searchParams.get("token") || "",
    nonce: parsed.searchParams.get("nonce") || "",
    apiBaseUrl: sanitizeBaseUrl(parsed.searchParams.get("apiBaseUrl") || ""),
    accountReference: "",
    partitionKey: parsed.searchParams.get("partitionKey") || loadAgentState().partitionKey || "",
    ...
    desktopAuthConfig: normalizeDesktopAuthConfig({
      readySelector:
        ...
        "",
      readyRejectSelector:
        parsed.searchParams.get("rejectSelector") ||
        "",
      readyUrlPattern:
...
function applyLaunchPayload(payload) {
  launchState = {
    flow: payload?.flow === "fbr" ? "fbr" : "dld",
    token: typeof payload?.token === "string" ? payload.token : "",
    nonce: typeof payload?.nonce === "string" ? payload.nonce.trim() : "",
    apiBaseUrl: sanitizeBaseUrl(typeof payload?.apiBaseUrl === "string" ? payload.apiBaseUrl : ""),
    accountReference:
      typeof payload?.accountReference === "string" ? payload.accountReference.trim() : "",
    partitionKey:
    ...
    allowedOrigins: Array.isArray(payload?.allowedOrigins)
      ? payload.allowedOrigins.filter((value) => typeof value === "string").map((value) => value.trim()).filter(Boolean)
      : [],
    backendAllowlist: Array.isArray(payload?.backendAllowlist)
      ? payload.backendAllowlist.filter((value) => typeof value === "string").map((value) => value.trim()).filter(Boolean)
      : [],
...
async function ensureTrustedDeviceRegistration() {
  if (!launchState.token || !launchState.apiBaseUrl) {
    throw new Error("No active connection session was provided by the web app.");
  }

  const response = await fetch(`${launchState.apiBaseUrl}${getTrustedDeviceRegisterEndpoint()}`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || !result?.ok) {
    throw new Error(
      result?.error || `Trusted device registration failed with status ${response.status}.`,
    );
  }
...
function getWorkerPartition() {
  const partitionKey = launchState.partitionKey || loadAgentState().partitionKey || "default";
...
async function ensureWorkerWindow() {
  ...
  workerWindow = new BrowserWindow({
    ...
    minWidth: 1100,
    minHeight: 760,
    title: launchState.flow === "fbr" ? "Tax Rocket Iris Dry Run" : "Ejari Local Automation",
...
async function collectPreFillComparison(windowInstance, portalFieldMap) {
  ...
  for (const field of portalFieldMap) {
    ...
    const selector = field.selector || `[data-tax-field-key="${String(field.key).replace(/"/g, '\\"')}"]`;
    const currentValue = await windowInstance.webContents.executeJavaScript(`
      (() => {
...
function getSelectorBundleSignal(jobContext) {
  ...
  const payloadBundle = jobContext?.job?.payload?.selectorBundle || jobContext?.payload?.selectorBundle;
...
function inferLikelySelectorGroup(errorMessage, executionLog) {
  ...
  const recentStep = Array.isArray(executionLog) && executionLog.length > 0
  ...
  if (step === "completion_verification" || message.includes("completed tasks") || message.includes("acknowledgement")) {
...
function classifyRecoverableAssistedIssue(errorMessage, executionLog, jobContext) {
  const selectorDriftDiagnostics = buildSelectorDriftDiagnostics(errorMessage, executionLog, jobContext);
  if (selectorDriftDiagnostics) {
    return {
      ...
      pauseReason: "Selector drift suspected. Update the route selector bundle before continuing.",
      userInstruction: "Update the affected selector bundle entry, then confirm to retry this phase.",
  ...
  if (looksLikeSessionReconnectNeeded(errorMessage)) {
    return {
      ...
      message: "Trusted Iris session needs to be reconnected before assisted filing can continue.",
      pauseReason: "The local Iris session is no longer valid for this phase.",
      userInstruction: "Sign back into IRIS on the trusted device, return to the expected screen, then confirm to continue.",
...
function buildRecoveryActions(errorMessage, options = {}) {
  ...
  if (options.selectorDriftDiagnostics) {
    const likelyGroup = options.selectorDriftDiagnostics.likelySelectorGroup || "unknown group";
    return [
...
async function waitForVisibleSelector(windowInstance, selector, timeoutMs = 15000) {
 ...
 */
async function trySelectorsInPriority(windowInstance, action, selectors, driftContext) {
...
async function reportSelectorDriftToServer(driftData) {
  try {
    ...
    const deviceAuthToken = launchState.deviceAuthToken || loadAgentState().deviceAuthToken;
    if (!deviceAuthToken) return;

    const reportUrl = new URL("/api/agents/selector-drift-reports", apiBaseUrl).toString();
...
async function setFileInputFiles(windowInstance, selector, filePaths) {
  ...
  const objectIdResponse = await windowInstance.webContents.debugger.sendCommand(
    "Runtime.evaluate",
    {
  ...
  await windowInstance.webContents.debugger.sendCommand("DOM.setFileInputFiles", {
...
async function downloadJobDocuments(jobId, documents) {
  ...
  for (const document of documents || []) {
    ...
    if (!response.ok) {
      throw new Error(`Failed to download document "${document.fileName}" for local automation.`);
    }
...
async function updateLocalJobStatus(jobId, status, body = {}) {
  ...
  if (!deviceAuthToken || !apiBaseUrl) {
    throw new Error("Trusted desktop device credentials are not ready for status updates.");
  }
...
async function loadLocalJobContext(jobId) {
  ...
  if (!deviceAuthToken || !apiBaseUrl) {
    throw new Error("Trusted desktop device credentials are not ready for job loading.");
  }
...
async function runLocalDldFlow(jobContext, localDocuments) {
  ...
  if (config.validationRejectSelector && (await hasSelector(windowInstance, config.validationRejectSelector))) {
    throw new Error("The local MyDLD session appears invalid and requires reconnect.");
  }
  ...
  try {
    ...
    await fillSelector(windowInstance, fields.tenantName, registration.tenantName);
    await fillSelector(windowInstance, fields.landlordName, registration.landlordName);
    await fillSelector(windowInstance, fields.rentAmount, registration.rentAmount);
    await fillSelector(windowInstance, fields.contractStartDate, registration.contractStartDate);
    await fillSelector(windowInstance, fields.contractEndDate, registration.contractEndDate);
    await fillSelector(windowInstance, fields.unitAddress, registration.unitAddress);
    await fillSelector(windowInstance, fields.makaniNumber, registration.makaniNumber);
    await fillSelector(windowInstance, fields.previousEjariId, registration.previousEjariId);
    ...
    await setFileInputFiles(windowInstance, uploads.tenancyContract, tenancyContract?.localPath ? [tenancyContract.localPath] : []);
    await setFileInputFiles(windowInstance, uploads.emiratesId, emiratesId?.localPath ? [emiratesId.localPath] : []);
    await setFileInputFiles(windowInstance, uploads.dewaBill, dewaBill?.localPath ? [dewaBill.localPath] : []);
    await setFileInputFiles(windowInstance, uploads.titleDeed, titleDeed?.localPath ? [titleDeed.localPath] : []);
    ...
    await new Promise((resolve) => setTimeout(resolve, config.postSubmitWaitMs || 5000));

    if (config.successSelector) {
      await waitForVisibleSelector(windowInstance, config.successSelector, 15000);
      ...
      throw new Error("A local success selector or success URL fragment is required.");
    }
...
async function clickClassicTreePanel(windowInstance, headerText) {
  ...
  if (clicked) {
    ...
    console.warn(`[classic-portal] clickClassicTreePanel("${headerText}") — no matching header found`);
  }
...
async function clickClassicTreeMenuItem(windowInstance, itemText) {
  ...
  if (clicked) {
    ...
    console.warn(`[classic-portal] clickClassicTreeMenuItem("${itemText}") — no matching item found`);
  }
...
async function fillClassicDataTable(windowInstance, rowDesc, fieldType, value) {
  ...
  if (!filled) {
    console.warn(`[classic-portal] fillClassicDataTable("${rowDesc}", "${fieldType}", "${value}") — could not find matching row/field`);
  }
...
async function navigateNewPortalPreRedirectFlow(windowInstance) {
  ...
  try {
    const normalReturnClicked = await windowInstance.webContents.executeJavaScript(`
      (() => {
    ...
    console.warn("[classic-portal] FBR5 return type selection failed:", err.message);
  }
  ...
  try {
    const residencyClicked = await windowInstance.webContents.executeJavaScript(`
      (() => {
    ...
    console.warn("[classic-portal] FBR7 residency declaration failed:", err.message);
  }
  ...
  try {
    ...
    console.warn("[classic-portal] FBR8 redirect acceptance failed:", err.message);
  }
...
function resolveClassicSectionPath(irisSection) {
  ...
  if (s.includes("attribut") || s.includes("business sector") || s.includes("residence")) {
...
async function navigateToClassicSection(windowInstance, irisSection) {
  ...
  if (!path) {
    console.warn(`[classic-portal] No tree path mapped for section: "${irisSection}"`);
    return false;
  }

  const panelExpanded = await clickClassicTreePanel(windowInstance, path.panel);
  if (!panelExpanded) {
    console.warn(`[classic-portal] Could not expand panel "${path.panel}" for section "${irisSection}"`);
  }

  const itemClicked = await clickClassicTreeMenuItem(windowInstance, path.item);
  if (itemClicked) {
    console.log(`[classic-portal] Navigated to section: ${path.panel} \u2192 ${path.item}`);
    return true;
  }

  console.warn(`[classic-portal] Could not click item "${path.item}" in panel "${path.panel}"`);
  return false;
}
 ...
 */
async function navigateClassicPortalFlow(windowInstance, routeSelector, useMockIris) {
  const steps = [];
  ...
  const preRedirectResult = await navigateNewPortalPreRedirectFlow(windowInstance);
  steps.push(...preRedirectResult.steps.map(s => `preredirect_${s}`));
  steps.push("awaiting_classic_portal");

  if (!preRedirectResult.success) {
    console.warn("[classic-portal] Pre-redirect flow may not have completed all screens. Proceeding anyway...");
  }
...
async function runLocalTaxDryRunFlow(jobContext) {
  ...
  const portalFieldMap = Array.isArray(snapshot.portalFieldMap) ? snapshot.portalFieldMap : [];
  const executionLog = [];
  ...
  if (!portalFieldMap.length) {
    throw new Error("The approved filing packet does not contain a portal field map for dry-run fill.");
  }

  const readySelector = config?.readiness?.readySelector || "#iris-dashboard-ready";
  ...
  executionLog.push({
    ...
    detail: "Desktop worker validated the configured ready screen before entering dry-run data.",
  });

  const captures = [await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.READINESS)];
  ...
  if (config.useMockIris) {
    ...
    const navigationResult = await navigateClassicPortalFlow(
      windowInstance, routeSelector, Boolean(config.useMockIris),
    );
    ...
    captures.push(await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.ROUTE_NAVIGATION));

    if (!navigationResult.formReady) {
      throw new Error("Classic portal navigation completed but the target data table was not detected as ready.");
    }
    ...
    if (!navigationResult.formReady) {
      throw new Error("Route-aware navigation completed but the target form was not detected as ready.");
    }
  } else {
  ...
  if (isClassicRoute) {
    ...
    for (const [sectionKey, fields] of sectionGroups) {
      ...
      const rowCount = await windowInstance.webContents.executeJavaScript(`
        (() => {
      ...
      for (const field of fields) {
        ...
        const filled = await fillClassicDataTable(windowInstance, rowDesc, fieldType, field.value);
  ...
  captures.push(await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.FIELD_FILL));

  const reviewGateSelector = config?.dryRun?.reviewGateSelector || "#dry-run-review-gate";
  await waitForVisibleSelector(windowInstance, reviewGateSelector, 15000);
  captures.push(await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.DRY_RUN_REVIEW_GATE));

  const finalSubmitSelector = config?.dryRun?.finalSubmitSelector || "#final-submit";
  const finalSubmitVisible = await hasSelector(windowInstance, finalSubmitSelector);
  ...
  return {
    result: {
      message: "Local Iris dry run completed and stopped at the final review gate.",
      pauseReason:
...
function getPilotStateFromContext(jobContext) {
  ...
  if (payload && typeof payload === "object" && payload.livePilotState && typeof payload.livePilotState === "object") {
...
async function pauseAssistedPilot(job, windowInstance, input) {
  const captures = [await captureWindowScreenshot(windowInstance, input.captureLabel)];
  await updateLocalJobStatus(job.id, "awaiting_user_action", {
    result: {
      ...
      captures,
      ...(input.extraResult && typeof input.extraResult === "object" ? input.extraResult : {}),
    },
...
async function runLocalTaxAssistedFilingFlow(jobContext, job) {
  ...
  if (!portalFieldMap.length) {
    throw new Error("The approved filing packet does not contain a portal field map for assisted filing.");
  }

  const readySelector = config?.readiness?.readySelector || "#iris-dashboard-ready";
  const dashboardUrl = resolveMockIrisUrl(assistedConfig.readinessUrl || "mock-iris://dashboard");
  await windowInstance.loadURL(dashboardUrl);
  await waitForVisibleSelector(windowInstance, readySelector, 15000);
  executionLog.push({
    step: STANDARD_LOG_STEPS.READINESS_CHECK,
    label: "Trusted Iris session confirmed",
    detail: "Desktop worker validated the trusted local Iris session before entering the live pilot.",
  });
  ...
  const prefillComparison = await collectPreFillComparison(windowInstance, portalFieldMap);
  ...
  captures.push(await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.PREFILL_COMPARE));
  captures.push(await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.FIELD_FILL));

  // ── Phase 15.5c F9: Classic portal has no mid-filing password reset ──
  if (pilotState.phase === "start" && !isClassic) {
    await windowInstance.loadURL(resolveMockIrisUrl(assistedConfig.passwordResetUrl || "mock-iris://password-reset"));
    executionLog.push({
      step: STANDARD_LOG_STEPS.PASSWORD_RESET_PAUSE,
      label: "Paused for password reset",
      detail: "Pilot paused so the user can complete any Iris password reset locally.",
    });
    return pauseAssistedPilot(job, windowInstance, {
      ...
      pauseReason: "Complete any required Iris password reset locally before continuing.",
      userInstruction: "If Iris asked for a password reset, finish it on the trusted device, then continue here.",
      recoveryActions: [
  ...
  if (pilotState.phase === "after_password_reset" && !isClassic) {
    await windowInstance.loadURL(resolveMockIrisUrl(assistedConfig.otpCaptchaUrl || "mock-iris://otp-captcha"));
    executionLog.push({
      step: STANDARD_LOG_STEPS.OTP_CAPTCHA_PIN_PAUSE,
      label: "Paused for OTP, captcha, and PIN",
      detail: "Pilot paused so the user can handle OTP, captcha, and any Iris PIN gates locally.",
    });
    return pauseAssistedPilot(job, windowInstance, {
      ...
      pauseReason: "Complete the OTP, captcha, or Iris PIN step locally before continuing.",
      userInstruction: "Handle the verification prompts on the trusted device, then continue here.",
      recoveryActions: [
  ...
  if (pilotState.phase === "after_otp_captcha_pin" && !isClassic && Number(snapshot.returnSummary?.taxPayable || 0) > 0) {
    await windowInstance.loadURL(resolveMockIrisUrl(assistedConfig.paymentUrl || "mock-iris://payment"));
    executionLog.push({
      step: STANDARD_LOG_STEPS.PAYMENT_PSID_PAUSE,
      label: "Paused for PSID and payment",
      detail: "Pilot paused so the user can create a PSID or complete payment locally.",
    });
    return pauseAssistedPilot(job, windowInstance, {
      ...
      pauseReason: "Complete PSID creation or payment locally before continuing.",
      userInstruction: "Finish the PSID or payment step on the trusted device, then continue here.",
      recoveryActions: [
  ...
  if (isClassic) {
    if (pilotState.phase === "start" || pilotState.phase === "after_password_reset") {
      // Classic portal: go directly to final review (save→submit→confirm dialog)
      const classicConfig = config.classicAssistedFiling || {};
      await windowInstance.loadURL(resolveMockIrisUrl(classicConfig.finalReviewUrl || "mock-iris://classic-portal"));
      executionLog.push({
        step: STANDARD_LOG_STEPS.FINAL_SUBMIT_CONFIRMATION_GATE,
        label: "Paused for classic portal final review",
        detail: "Classic portal pilot paused at the final review gate (save → submit → confirm dialog).",
      });
      return pauseAssistedPilot(job, windowInstance, {
        requiredAction: "classic_final_review",
        captureLabel: STANDARD_CAPTURE_LABELS.FINAL_SUBMIT_CONFIRMATION,
        message: "Assisted filing paused at the classic portal final review gate.",
        pauseReason: "Complete the classic portal save → submit → confirm dialog locally, then continue for PIN entry.",
        userInstruction: "In the classic portal: click Save, then Submit, confirm the declaration dialog. Then continue here.",
        recoveryActions: [
    ...
    if (pilotState.phase === "after_otp_captcha_pin" || pilotState.phase === "after_payment_psid") {
      // Classic portal PIN entry: after submit confirmation, the portal shows a 4-digit PIN dialog
      const classicConfig = config.classicAssistedFiling || {};
      await windowInstance.loadURL(resolveMockIrisUrl(classicConfig.pinEntryUrl || "mock-iris://classic-pin"));
      executionLog.push({
        step: "classic_pin_entry_pause",
        label: "Paused for classic portal PIN entry",
        detail: "Classic portal pilot paused for 4-digit FBR PIN entry before final submission.",
      });
      return pauseAssistedPilot(job, windowInstance, {
        ...
        pauseReason: "Enter the 4-digit FBR PIN in the classic portal PIN dialog before final submission.",
        userInstruction: "Enter your FBR PIN, click Submit, then continue here for proof capture.",
        recoveryActions: [
  ...
  if (!isClassic && (
    ...
    await windowInstance.loadURL(resolveMockIrisUrl(assistedConfig.finalReviewUrl || "mock-iris://final-review"));
    executionLog.push({
      step: STANDARD_LOG_STEPS.FINAL_SUBMIT_CONFIRMATION_GATE,
      label: "Paused for final submit confirmation",
      detail: "Pilot stopped at the final declaration so the user can confirm the last action locally.",
    });
    return pauseAssistedPilot(job, windowInstance, {
      requiredAction: "final_submit_confirmation",
      captureLabel: STANDARD_CAPTURE_LABELS.FINAL_SUBMIT_CONFIRMATION,
      message: "Assisted filing paused at the final submit confirmation gate.",
      pauseReason: "User must confirm the final declaration locally before proof capture can continue.",
      userInstruction: "If you submitted locally, continue here so Tax Rocket can capture proof and close the pilot job.",
      recoveryActions: [
  ...
  if (routeSelector && !config.useMockIris && !isClassic) {
    ...
    const paymentVerification = await verifyCprAndPayment(windowInstance, routeSelector);
    executionLog.push({
    ...
    captures.push(await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.PAYMENT_VERIFICATION));

    // Enforce ready_to_submit gate: if payment is required and submit is not unlocked,
    // pause for user intervention
    if (Number(snapshot.returnSummary?.taxPayable || 0) > 0 && !paymentVerification.submitUnlocked) {
      return pauseAssistedPilot(job, windowInstance, {
        ...
        pauseReason: "Complete payment or verify CPR on the portal before continuing.",
        userInstruction: "Check the payment status on the trusted device and ensure the submit button is enabled.",
        recoveryActions: [
  ...
  await windowInstance.loadURL(resolveMockIrisUrl(assistedConfig.completedTasksUrl || "mock-iris://completed"));
  const proofCaptures = [
    await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.COMPLETED_TASKS),
    await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.ACKNOWLEDGEMENT),
    await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.RETURN_COPY),
    await captureWindowScreenshot(windowInstance, STANDARD_CAPTURE_LABELS.CPR_PROOF),
  ];
  ...
  if (routeSelector && !config.useMockIris) {
    completionEvidence = await verifyCompletionEvidence(windowInstance, routeSelector);
    executionLog.push({
  ...
  executionLog.push({
    ...
    detail: "Desktop worker captured Completed Tasks, acknowledgement, return copy, and CPR proof surfaces.",
  });
  ...
  return {
    result: {
      ...
      paymentVerification: routeSelector && !config.useMockIris
        ? await verifyCprAndPayment(windowInstance, routeSelector).catch(() => null)
        : null,
    },
    executionLog,
  };
...
async function processLocalJob(job) {
  ...
  pushStatus("progress", `Running local desktop automation for job ${job.publicId || job.id}.`);
  ...
  try {
    ...
    if (outcome?.paused) {
      pushStatus("progress", "Local desktop job is waiting for a user action before it can continue.");
      return;
    }
    ...
    const message =
      error instanceof Error ? error.message : "The local desktop automation failed unexpectedly.";
    ...
    const recoverableAssistedIssue =
      job.type === "tax_assisted_filing"
        ? classifyRecoverableAssistedIssue(message, failureExecutionLog, context)
        : null;
    ...
    if (recoverableAssistedIssue) {
      await updateLocalJobStatus(job.id, "awaiting_user_action", {
        result: {
          ...
          selectorDriftDiagnostics: recoverableAssistedIssue.selectorDriftDiagnostics,
      ...
      pushStatus("progress", "Assisted filing is waiting for a supervised recovery confirmation.");
      return;
    }

    const selectorDriftDiagnostics = buildSelectorDriftDiagnostics(message, failureExecutionLog, context);
...
async function runLocalWorkerCycle() {
  ...
  try {
    ...
    pushStatus(
      "error",
      error instanceof Error ? error.message : "The local desktop worker encountered an unexpected error.",
    );
  } finally {
...
function startLocalBridgeServer() {
  ...
  localBridgeServer = http.createServer(async (request, response) => {
    ...
    const originAllowed =
      !requestedAllowedOrigins.length || isOriginAllowed(requestOrigin, requestedAllowedOrigins);
    ...
    request.on("end", async () => {
      try {
        ...
        const backendAllowlist = Array.isArray(payload?.backendAllowlist)
          ? payload.backendAllowlist.filter((value) => typeof value === "string")
          : [];
        const bridgeOriginAllowed =
          !allowedOrigins.length || isOriginAllowed(requestOrigin, allowedOrigins);
        const backendAllowed = isBackendAllowed(payload?.apiBaseUrl, backendAllowlist);
        const nonce = typeof payload?.nonce === "string" ? payload.nonce.trim() : "";

        if (!bridgeOriginAllowed) {
          response.writeHead(403, { "Content-Type": "application/json" });
          response.end(JSON.stringify({ ok: false, error: "Bridge origin is not allowed." }));
          return;
        }

        if (!backendAllowed) {
          response.writeHead(403, { "Content-Type": "application/json" });
          response.end(JSON.stringify({ ok: false, error: "Desktop launch backend is not allowed." }));
          return;
        }

        if (!nonce || acceptedLaunchNonces.has(nonce)) {
          response.writeHead(409, { "Content-Type": "application/json" });
          response.end(JSON.stringify({ ok: false, error: "Desktop launch nonce is missing or already used." }));
          return;
        }
        ...
        if (confirmation.response !== 0) {
          response.writeHead(403, { "Content-Type": "application/json" });
          response.end(JSON.stringify({ ok: false, error: "Desktop launch was cancelled by the user." }));
          return;
        }
        ...
        if (!accepted) {
          ...
          response.end(JSON.stringify({ ok: false, error: "token and apiBaseUrl are required." }));
          return;
        }
        ...
        response.end(
          JSON.stringify({
            ok: false,
            error: error instanceof Error ? error.message : "Invalid launch payload.",
          }),
        );
      }
    });
...
async function attemptAutoCapture(reason) {
  ...
  if (!launchState.token || !launchState.apiBaseUrl) {
    pushStatus("error", "Return to the web app and start the connection again.");
    return;
  }
  ...
  try {
    ...
    pushStatus(
      "error",
      error instanceof Error ? error.message : "This desktop device could not be marked ready automatically.",
    );
  }
}
...
async function captureLoginWindowState() {
  ...
  const desktopAuthConfig = normalizeDesktopAuthConfig(launchState.desktopAuthConfig);
  ...
  if (desktopAuthConfig.readySelector) {
    await waitForVisibleSelector(loginWindow, desktopAuthConfig.readySelector, 5000);
  } else if (desktopAuthConfig.readyUrlPattern) {
    if (!matchesSuccessUrlPattern(currentUrl, desktopAuthConfig.readyUrlPattern)) {
      throw new Error(
        launchState.flow === "fbr"
...
async function markTrustedDeviceReady() {
  ...
  const response = await fetch(`${launchState.apiBaseUrl}${getTrustedDeviceReadyEndpoint()}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${deviceAuthToken}`,
    ...
    body: JSON.stringify({
      currentUrl: loginWindow && !loginWindow.isDestroyed() ? loginWindow.webContents.getURL() : null,
    }),
  });
  ...
  if (!response.ok || !result?.ok) {
    throw new Error(result?.error || `Ready-state update failed with status ${response.status}.`);
  }
...
ipcMain.handle("get-local-bridge-url", async () => `http://${LOCAL_BRIDGE_HOST}:${LOCAL_BRIDGE_PORT}/connect`);
  ...
  app.whenReady().then(() => {
    try {
      if (process.defaultApp) {
        app.setAsDefaultProtocolClient("ejari-connect", process.execPath, [path.resolve(process.argv[1])]);
